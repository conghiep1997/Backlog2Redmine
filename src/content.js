    ...
    for (const m of mutations) {
      for (const node of m.addedNodes) {
        if (node.nodeType === Node.ELEMENT_NODE) {
          // Check for comments
          if (node.matches?.("div.comment-item__actions")) injectButtonIfNeeded(node);
          else node.querySelectorAll?.("div.comment-item__actions").forEach(injectButtonIfNeeded);
          
          // Check for header (newly added)
          if (node.matches?.(".title-group__edit-actions")) injectHeaderButton();
          else if (node.querySelector?.(".title-group__edit-actions")) injectHeaderButton();
        }
      }
    }
  });
...
function injectHeaderButton() {
  ...
  if (!container || container.querySelector(".tb-header-migrate-btn")) return;
...
function injectButtonIfNeeded(actionsEl) {
  ...
  if (quoteBtn) quoteBtn.before(button); else actionsEl.appendChild(button);
...
async function handleTranslateAndOpenModal(actionsEl, button) {
  ...
  if (!issueKey) { showToast(TB.MESSAGES.TOAST.MISSING_ISSUE_KEY, "error"); return; }

  const allItems = Array.from(document.querySelectorAll(".comment-item"));
  const idx = allItems.findIndex(item => item === commentItem);
  const remainingItems = allItems.slice(idx + 1).map(i => ({ text: extractBacklogContent(i.querySelector("div.comment-content")), url: getCommentUrl(i) })).filter(c => c.text);
  ...
  try {
    const result = await sendRuntimeMessage({
      ...
      commentText: extractBacklogContent(commentItem.querySelector("div.comment-content")),
      commentUrl: getCommentUrl(commentItem)
    });

    openConfirmModal({
      redmineIssueId: result.redmineIssueId, issueTitle: result.issueTitle, previewText: result.previewText,
      remainingComments: remainingItems, hasBatchOption: remainingItems.length > 0,
      onCancel: () => setButtonLoading(button, false),
      onConfirm: async ({ redmineIssueId, notesList }) => {
        let lastRes;
        for (const notes of notesList) lastRes = await sendRuntimeMessage({ type: "SEND_TO_REDMINE", redmineIssueId, notes });
        openSuccessModal({ redmineUrl: lastRes.redmineUrl, commentCount: notesList.length, onClose: () => setButtonLoading(button, false) });
      },
      translateBatch: (comments) => Promise.all(comments.map(c => sendRuntimeMessage({ type: "LOOKUP_AND_TRANSLATE_COMMENT", issueKey, issueSummary, commentText: c.text, commentUrl: c.url }).then(r => r.previewText)))
    });
    setButtonLoading(button, false);
  } catch (err) { setButtonLoading(button, false); showToast(err.message, "error"); }
}
...
async function handleIssueMigration(button) {
  ...
  const comments = Array.from(document.querySelectorAll(".comment-item:not(.-dammy)")).map(i => ({ text: extractBacklogContent(i.querySelector("div.comment-content")), url: getCommentUrl(i) })).filter(c => c.text);
  ...
  try {
    const res = await sendRuntimeMessage({
      type: "LOOKUP_AND_TRANSLATE_COMMENT",
      issueKey, issueSummary,
      commentText: extractBacklogContent(document.querySelector("#issueDescription .markdown-body")),
      commentUrl: window.location.href
    });

    openConfirmModal({
      isMigration: true, issueTitle: [issueKey, issueSummary].filter(Boolean).join(" "), previewText: res.previewText,
      commentsCount: comments.length,
      onCancel: () => setButtonLoading(button, false),
      onConfirm: async ({ issueData }) => {
        const result = await sendRuntimeMessage({ type: "CREATE_REDMINE_ISSUE", issueData, comments: comments.map(c => c.text) });
        openSuccessModal({ redmineUrl: result.redmineUrl, commentCount: comments.length + 1, onClose: () => setButtonLoading(button, false) });
      }
    });
    setButtonLoading(button, false);
  } catch (err) { setButtonLoading(button, false); showToast(err.message, "error"); }
...
function getBacklogHeaderInfo() {
  ...
  const issueSummary = document.querySelector('[data-testid="issueSummary"]')?.textContent?.trim() || "";
...
function getCommentUrl(item) {
  ...
  if (!id) return null;
  if (id.startsWith("comment-item-")) id = id.replace("comment-item-", "comment-");
