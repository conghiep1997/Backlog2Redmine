    ...
    for (const m of mutations) {
      for (const node of m.addedNodes) {
        if (node.nodeType === Node.ELEMENT_NODE) {
          if (node.matches?.("div.journal div.contextual")) injectButtonIfNeeded(node);
          else node.querySelectorAll?.("div.journal div.contextual").forEach(injectButtonIfNeeded);
        }
      }
    }
  });
...
function injectButtonIfNeeded(actionsEl) {
  ...
  button.onclick = (e) => { e.preventDefault(); e.stopPropagation(); handleExtractAndOpenModal(actionsEl, button); };
...
async function handleExtractAndOpenModal(actionsEl, button) {
  ...
  if (!rawText) { showToast("Không tìm thấy nội dung bình luận.", "error"); return; }
  ...
  try {
    ...
    openBacklogModal({
      backlogIssueKey, previewText: result.previewText,
      onCancel: () => setButtonLoading(button, false),
      onConfirm: async ({ backlogIssueKey, content, notifiedUserId }) => {
        const sendResult = await sendRuntimeMessage({ type: "SEND_TO_BACKLOG", backlogIssueKey, content, notifiedUserId });
        openSuccessModal({ redmineUrl: sendResult.backlogUrl, onClose: () => setButtonLoading(button, false) });
      }
    });
...
function findBacklogKeyInPage() {
  ...
  for (const p of patterns) { const m = desc.match(p); if (m) return m[1]; }
  return "";
}
