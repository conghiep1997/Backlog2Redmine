...
function parseChangelog(content) {
  ...
  while ((match = versionRegex.exec(content)) !== null) {
    ...
    const sectionEnd = nextVersionMatch
      ? sectionStart + 1 + nextVersionMatch.index
      : content.length;
    ...
    const firstFeature =
      changelog.length > 0 ? changelog[0].replace("- ", "") : "Bug fixes and improvements";
    const description =
      firstFeature.length > 200 ? firstFeature.substring(0, 197) + "..." : firstFeature;
  ...
  for (let i = 0; i < reversedVersions.length; i++) {
    ...
    try {
      ...
      if (response.ok) {
        const result = await response.json();
        console.log(
          `   ✓ Created version ${version.version_number} (ID: ${result.id})${isLatest ? " ⭐ LATEST" : ""}`
        );
      } else {
        const errorText = await response.text();
