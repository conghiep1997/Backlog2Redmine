...
function isBackendRegistrationRequired() {
  return /^true$/i.test(process.env.REQUIRE_BACKEND_VERSION_REGISTRATION || "");
}

function skipBackendRegistration(message) {
  if (isBackendRegistrationRequired()) {
    console.error(message);
    process.exit(1);
  }

  console.warn(message + " Skipping optional backend version registration.");
}
...
async function registerVersion() {
  const { BACKEND_URL: backendUrl, VERSION: version, DOWNLOAD_URL: downloadUrl } = process.env;

  // Validate required environment variables
  if (!backendUrl) {
    skipBackendRegistration("Missing BACKEND_URL environment variable.");
    return;
  }
  ...
  const description =
    extractReleaseDescription(changelogContent, version) || extractDescription(changelog);
  ...
  try {
    ...
    if (response.ok) {
      ...
      const message = "Backend API error (" + response.status + "): " + errorText;
      skipBackendRegistration(message);
    }
  } catch (error) {
    skipBackendRegistration("Failed to register version: " + error.message);
  }
}
...
module.exports = {
  extractReleaseDescription,
  isBackendRegistrationRequired,
  parseChangelogToStructured,
};
